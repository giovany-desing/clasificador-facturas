...
def predecir_archivo(ruta_archivo):
    ...
    try:
        ...
        imagen_preprocesada = preprocessing_data.extraer_caracteristicas_imagen_preprocesamiento(ruta_archivo)
    ...
    try:
        ...
        for i, archivo in enumerate(archivos, 1):
            ...
            if clase is not None:
                # Log según la clase
                if clase == 0:
                    logger.info(f"🔌 CONECTAR A 0 - {archivo} (Probabilidad: {probabilidad:.4f})")
                    conectar_0_count += 1
                else:  # clase == 1
                    logger.info(f"🔌 CONECTAR A 1 - {archivo} (Probabilidad: {probabilidad:.4f})")
                    conectar_1_count += 1
        ...
        logger.info(f"🔌 CONECTAR A 0 (carpeta 'corr'): {conectar_0_count} archivos")
        logger.info(f"🔌 CONECTAR A 1 (carpeta 'prev'): {conectar_1_count} archivos")
        ...
        if len(resultados) > 0:
            ...
            logger.info(f"📊 Distribución: {porcentaje_0:.1f}% conectar a 0, {porcentaje_1:.1f}% conectar a 1")
        ...
        logger.info(f"📁 Archivos clase 0 en: {CARPETA_CORR}")
        logger.info(f"📁 Archivos clase 1 en: {CARPETA_PREV}")
